...
public class ComHandler
{
	...
	private MotorCommand computeDirectionCommand(IDirection command)
	{
		...
		*/
	    ...
	    // To compute direction it i reauired only the degrees in interval [-90, 90]
	    // -90 means 90 degrees to left
	    //  90 means 90 degrees to right
	    MotorCommand mC;
	    int degrees = command.getDegree();
	    int pause = 2000;
	    int duration = 3000;
	    
	    if(degrees < 0)    // it -degrees rotate to left
	        mC = new MotorCommand(command, pause, 0, degrees, duration, 0, 0 );
	    else
	        mC = new MotorCommand(command, pause, degrees, 0, duration, 0, 0);
	    return mC;
	}
	...
	private MotorCommand computeGearCommand(IGear command)
	{
		...
		*/
	    ...
	    // Tom compute Gear, SPEED and DURATION are required.
	    // Where speed must be in interval [-100, 100];
	    // Negative speeds means backward, positive speeds means forward
	    // 0 is minimum speed and 100 is maximum speed 
	    // Duration must be in milliseconds
	    MotorCommand mC;
	    int duration = (int)(command.getDuration() * 1000); //convert duation to milliseconds
	    int speed = command.getSpeed();
	    int pause = (int) (2000 + command.getDuration() * 1000);
	    
	    if(speed < 0)
	        mC = new MotorCommand(command, pause, speed, 0, duration, 0, 0);
	    else
	           mC = new MotorCommand(command, pause, 0, speed, duration, 0, 0);
	    return mC;
	}
	...
	private MotorCommand computePauseCommand(IPause command)
	{
		...
		MotorCommand mC = new MotorCommand(command, pause, pause, 0,
				(int) command.getDuration() * 1000, 0, 0);
		return mC;
	}
